...
class DemoqaTest {
    ...
    fun happyTest(browserConfig: String) {
        ...
        Page.getInstance(NestedFramesPage::class).shouldLoadRequired().framesWrapper.parentFrame.switch {
    ...
    fun badTest(browserConfig: String) {
        setupSite(browserConfig)
        val requiredError =
            assertThrows<RequiredError> { Page.getInstance(BadNestedFramesPage::class).shouldLoadRequired() }
    ...
    fun switchTest(browserConfig: String) {
        setupSite(browserConfig)
        val nestedFramesPage = Page.getInstance(NestedFramesPage::class)
